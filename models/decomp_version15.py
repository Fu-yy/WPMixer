...
class WaveletODE_Pro(nn.Module):
    def __init__(self, in_channels, hidden_dim=64, time_dim=16):
        ...
        self.net = nn.Sequential(
            nn.Conv1d(in_channels, hidden_dim, 3, padding=1, padding_mode='replicate'),
            nn.GELU(),
            nn.Conv1d(hidden_dim, hidden_dim, 3, padding=1, padding_mode='replicate'),
            nn.GELU(),
            nn.Conv1d(hidden_dim, in_channels, 3, padding=1, padding_mode='replicate'),
            nn.Tanh()  # 输出约束防止发散
        )
    ...
    def forward(self, t, x):
        ...
        gamma, beta = t_params.chunk(2, dim=1)
        ...
        for i, layer in enumerate(self.net):
            if i == 0:  # 第一层
                h = layer(h)
            elif i < len(self.net) - 1:  # 中间层
                # 扩展gamma和beta以匹配h的形状
                gamma_exp = gamma.unsqueeze(-1).expand(-1, -1, h.size(-1))
                beta_exp = beta.unsqueeze(-1).expand(-1, -1, h.size(-1))
                h = h * (1 + gamma_exp) + beta_exp
                h = layer(h)
            else:  # 最后一层
                h = layer(h)

        # 添加阻尼项：dx/dt = f(x) - λx^3 (防止数值发散)
