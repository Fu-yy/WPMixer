...
class WPMixerCore(nn.Module):
    def __init__(self, 
        ...
        self.decomp = AdvancedWaveletDecomp(channel=self.configs.c_in, level=self.level, filter_length=5,dim=self.d_model,ode_solver='rk4', ode_step=0.2)
    ...
    def forward(self, xL):
        ...
        # yA = self.resolutionBranch[0](yl)
        # yD = []
        # for i in range(len(yh)):
        #     yD_i = self.resolutionBranch[i + 1](yh[i])
        #     # yD_i = yh[i]
