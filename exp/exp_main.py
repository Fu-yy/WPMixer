...
class Exp_Main(Exp_Basic):
    ...
    def _build_model(self):
        ...
        model = model_dict[self.args.model](self.args,self.device
                                        ).float()
    ...
    def get_gflops(self):
        ...
        input_tensor = torch.randn(batch, seq, channel).to('cuda')  # Dummy input

        self.model.eval()
        macs, params = profile(self.model, inputs=(input_tensor,), verbose=True)
        gflops = 2 * macs / 1e9  # MACs → FLOPs → GFLOPs（假设 macs 是乘加次数）
        gparams = params / 1e9  # 直接转换为 GParams
        print(f"Total GFLOPs: {gflops:.4f}")
        print(f"Total GPARAMs: {gparams:.4f}")
        return gflops, gparams
